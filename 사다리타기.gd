...
func 사다리풀이그리기() -> void:
	...
	for 참가자번호 in 칸수.x:
		...
		# 시작 세로줄 그리기
		사다리풀이.add_child(
			화살표.instantiate().init_2_point(세로줄위치(현재줄번호,0), 가로줄위치(현재줄번호,0), 참가자색[참가자번호], 화살표두께, 화살표날개길이, 화살표날개폭) )
		for y in 칸수.y:
			if 사다리자료[현재줄번호][y].왼쪽연결길 == true: # 왼쪽으로 한칸 이동
				# 현재까지의 세로줄 그리기
				사다리풀이.add_child(
					화살표.instantiate().init_2_point(가로줄위치(현재줄번호,oldy), 가로줄위치(현재줄번호,y), 참가자색[참가자번호], 화살표두께, 화살표날개길이, 화살표날개폭) )
				#왼쪽 화살표
				사다리풀이.add_child(
					화살표.instantiate().init_2_point(가로줄위치(현재줄번호,y)-shift, 가로줄위치(현재줄번호-1,y)-shift, 참가자색[참가자번호], 화살표두께, 화살표날개길이, 화살표날개폭) )
				현재줄번호 -= 1
				if 현재줄번호 < 0:
					현재줄번호 += 칸수.x
					사다리풀이.add_child(
						화살표.instantiate().init_2_point(가로줄위치(현재줄번호+1,y)-shift, 가로줄위치(현재줄번호,y)-shift, 참가자색[참가자번호], 화살표두께, 화살표날개길이, 화살표날개폭) )
				oldy = y
				continue
		...
		# 나머지 끝까지 그린다.
		세로줄 = 화살표.instantiate()
		세로줄.init_2_point(가로줄위치(현재줄번호,oldy), 가로줄위치(현재줄번호,칸수.y)-세로위치보정, 참가자색[참가자번호], 화살표두께, 화살표날개길이, 화살표날개폭)
		사다리풀이.add_child(세로줄)
